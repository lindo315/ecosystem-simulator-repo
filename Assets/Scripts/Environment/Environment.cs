...
public class Environment : MonoBehaviour {
    ...
    public static Surroundings Sense(Coord coord)
    {
        ...
        if (speciesMaps != null && speciesMaps.ContainsKey(Species.Plant))
        {
            surroundings.nearestFoodSource = speciesMaps[Species.Plant].ClosestEntity(coord, Animal.maxViewDistance);
        }

        if (closestVisibleWaterMap != null)
        {
            surroundings.nearestWaterTile = closestVisibleWaterMap[coord.x, coord.y];
        }
        else
        {
            surroundings.nearestWaterTile = Coord.invalid;
        }
