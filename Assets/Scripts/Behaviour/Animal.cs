...
public class Animal : LivingEntity {
    ...
    void HandleInteractions()
    {
        if (currentAction == CreatureAction.Eating)
        {
            if (foodTarget && hunger > 0)
            {
                ...
                // Let's modify this to check if foodTarget is actually a Plant
                if (foodTarget is Plant plantFood)
                {
                    eatAmount = plantFood.Consume(eatAmount);
                    hunger -= eatAmount;
                }
                else
                {
                    Debug.LogWarning("Food target is not a Plant. Resetting action.");
                    currentAction = CreatureAction.Exploring;
                    foodTarget = null;
                }
            }
    ...
    void OnDrawGizmosSelected()
    {
        if (Application.isPlaying)
        {
            ...
            if (surroundings != null)  // Add null check for surroundings
            {
                ...
                {
                    // Add null check for Environment.tileCentres
                    if (Environment.tileCentres != null)
                    {
                        Gizmos.DrawLine(transform.position,
                            Environment.tileCentres[surroundings.nearestWaterTile.x, surroundings.nearestWaterTile.y]);
                    }
                }
            }

            if (currentAction == CreatureAction.GoingToFood && foodTarget != null)
            {
                var path = EnvironmentUtility.GetPath(coord.x, coord.y, foodTarget.coord.x, foodTarget.coord.y);

                if (path != null)  // Add null check for path
                {
